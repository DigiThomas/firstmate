...
One shape is only partly contained, and the limit differs by the tail after the dot.
...
With a NON-NUMERIC tail such as `fm-task.a` alongside a window named `fm-task`, tmux resolves to `fm-task`, the pane component does not verify, and the target reports unreadable, which licenses no recovery.
With a NUMERIC tail such as `fm-task.0`, the tail is a legal pane index, so it verifies against the wrong window's pane and the target reports present about a different worker's pane.
Verified on tmux 3.6b with the live agent in `fm-task.0`: the endpoint reports present, the agent state reports dead for the neighbour's idle shell, and the bootstrap sweep acts on dead by killing that endpoint and respawning, so a live agent can read dead and an unrelated window can be destroyed.
The tmux backend refuses to create a window whose name contains a dot, which prevents new pairs, but it does not detect or migrate a pre-existing `fm-<id>` and `fm-<id>.<digits>` pair, and task ids may legally contain a dot.
The numeric case is therefore open rather than closed, it is pre-existing rather than introduced by the shared parser, and closing it is a behavioural change tracked separately.
Preferring the prefix window instead would confirm liveness about a different worker's pane on purpose, and steering that target would type into it.
