...
Closing it structurally belongs to publishing the lock identity at claim time, which removes the settling state and this class with it, and is tracked separately.
...
A successor is not healthy on its very first sample: `fm_lock_claim` publishes the lock's pid the instant the claim is taken, and the claiming watcher writes `fm-home`, `watcher-path`, and `pid-identity` only afterwards, so a healthy successor is briefly observable as a live lock pid with no published identity.
That observation is named in the arm as SETTLING rather than being waited out on timing: a lock whose pid names a live process while its identity is not published yet keeps the verification window polling, while a dead or absent lock pid, a lock that already names another home or another watcher path, and a fully published identity that does not match all remain unhealthy and still fail fast.
The window, `FM_ARM_ATTACH_RETARGET_MAX`, and `FM_ARM_RESTART_MAX` still bound it, so a lock that never finishes settling ends in the honest failure `the watcher lock holder pid=<N> never finished publishing this home's watcher identity within <W>s` instead of looping.
...
It exhausts the retarget budget against two live identity-matched holders to prove the arm abandons that attach rather than restarting over the healthy holder, and it holds a lock unpublished past the settling deadline to prove that tolerance stays bounded rather than polling forever.
