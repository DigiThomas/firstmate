...
The repositioning move-to-last preserves every surviving workspace's relative order, and an unconfirmed removal rolls the doomed workspace back to its verified original position under the same session lock, so no presentation ordering change outlives the attempt.
