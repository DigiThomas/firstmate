...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; kind=secondmate also records home= and projects=; a spawn that declared an existing branch records branch=, and one dispatched through the concurrent-session override records branch_collision_override=1, so an overridden dispatch stays visible after the fact (section 7); a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for an X-mode-originated task (section 14)
...
It also refuses when another live session is already working the task's branch, so declare an existing branch with `--branch <name>` and treat that refusal as a stop-and-reconcile result rather than reaching for its named override.
